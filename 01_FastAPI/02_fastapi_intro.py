...
@app.post("/add_people")
def add_people_diff(person: Person):
    people.append(person)
    return {"message": "Person added successfully", "people": people}


"""
In FastAPI, Uvicorn is the lightning-fast web server that actually runs your 
application. While FastAPI defines the routes, logic, and data validation, 
it does not have a built-in server to listen for network requests; it relies 
on Uvicorn to handle that low-level communication.

To run the FastAPI application, you typically use Uvicorn from the command 
line like this:
uvicorn main:app 
or 
...
"""
Pydantic:
    Python Library used for Data validation + data parsing using Python 
    classes(BaseModel). It validates incoming data, Converts data types 
    automatically(e.g., "25" → 25) and throws clear errors if data is invalid.

Pydantic is used in FastAPI to define the structure of request bodies, 
query parameters, and response models.
"""
